...
echo installing egi fedcloud clients
...
install_rh6() {
    ...
    # [Tomas Kulhanek] updated to 3.14.4-1 the UMD release per availability on the repository.egi.eu, can we use more secure HTTPS?
    ...
    # [Tomas Kulhanek] added SL 7 support - it seems there exists for SL 7 
    if [ "x$REV" == "x7" ]; then
    ...
    # [Tomas Kulhanek] broken dependencies - cernvm vs. epel packages, if skipped, it fails on conflict in /etc/grid-security/certificates
    yum -q -y install --skip-broken ca-policy-egi-core fetch-crl occi-cli voms-clients3
}
